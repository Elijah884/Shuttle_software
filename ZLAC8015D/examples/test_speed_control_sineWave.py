...
while True:
	try:
		...
		l_cur, r_cur = motors.get_currents()
		rpmL_fb, rpmR_fb = motors.get_rpm()
		print("period: {:.5f} t: {:.3f} rpmL_cmd: {:.2f} rpmR_cmd: {:.2f} rpmL_fb: {:.2f} rpmR_fb: {:.2f} left_cur: {:.1f}A right_cur: {:.1f}A".format(\
				period, t, rpmL_cmd, rpmR_cmd, rpmL_fb, rpmR_fb, l_cur, r_cur))
